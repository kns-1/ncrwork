...
DWORD WINAPI myFun1(LPVOID blah)
{
	...
	for (int i = 0; i < 5; i++)
	{
		cout << "\nONE: " << InterlockedIncrement(&sharedValue) << endl;
		Sleep(1000);
	}
	return 0;
}
DWORD WINAPI myFun2(LPVOID blah)
{
	for (int i = 0; i < 5; i++)
	{
		cout << "\nTWO: " << InterlockedIncrement(&sharedValue) << endl;
		Sleep(1000);
	}
	return 0;
}
void main()
{
	...
	{
		cout << "\nCreateThread() failed, Error code is: " << GetLastError() << endl;
		system("pause");
		exit(0);
	}
	...
	cout << "\nFinal Shared Value is: " << sharedValue << endl;
